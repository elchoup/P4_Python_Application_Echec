...
def select_players():
    ...
    We load the list of players, choose the number of players we want
    to register
    to the tournament en enter lastname and firstname to find the player.
    If the player is not in the competition list we send an error same as
    if we already registered the player in the list
    ...
    while i <= number_of_players:
        ...
        for resgistered in data:
            if (player_lastname.lower() == resgistered["Nom"]
                    and player_firstname.lower() == resgistered["Prenom"]):
                player = Player(resgistered["Nom"], resgistered["Prenom"],
                                resgistered["Age"])
...
def create_tournament():
    """This fonction is made to Create a tournament while entering all
